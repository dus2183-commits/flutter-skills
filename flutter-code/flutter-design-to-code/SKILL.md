...
description: |
  Figma/Zeplin 设计稿 → Flutter 代码。通过 MCP 调用设计工具 API 提取样式，生成代码。
  触发场景：用户说"根据 Figma 生成页面"、"把这个设计稿变成代码"、"从 Zeplin 切图"。
type: skill
stage: 3
model: opus
priority: P1
...
1. `{page_name}_page_ui.dart` — UI 代码
...
生成 1-2 个文件:
1. `lib/features/{module}/presentation/ui/{page_name}_ui.dart` — UI Widget 代码
...
```dart
// lib/features/announcement/presentation/ui/announcement_detail_ui.dart
...
**处理:** 在 `lib/core/theme/app_colors.dart` 新增这 3 个颜色。
...
**处理:** 在 `lib/core/theme/app_text_styles.dart` 新增这 2 个字体规范。
...
**处理:** 在 `lib/core/theme/app_spacing.dart` 新增。
...
> 
> **生成的文件:**
> - UI 代码: lib/features/{module}/presentation/ui/{page}_ui.dart
