...
description: 已有 Repository 代码 → 前端友好的接口使用文档。用户说"生成接口文档"、"这个接口怎么调"、"整理接口清单"时触发。和 api-design 的区别:api-design 是设计阶段输出契约,api-doc 是代码已有后输出使用文档。
type: skill
stage: 6
model: sonnet
priority: P2
...
> ⚠️ 本段仅作参考说明。实际代码生成请用对应 skill:
> - Model → `flutter-model-gen` (freezed 格式)
> - Repository → `flutter-api-gen` (ApiClient 调用)
> - 一键全套 → `flutter-api-quick`

**Model 路径:** `lib/features/{module}/data/models/{entity}.model.dart` (freezed 格式)

**Repository 路径:** `lib/features/{module}/data/repositories/{module}_repository.dart`

**关键规则:**
- path **不带 /api 前缀** (baseUrl 已含 apiPrefix,重复会 /api/api/ 404)
- Repository 继承 `GetxService`,通过 `Get.find<ApiClient>()` 获取实例
- 必须传 `mockKey` 和 `cancelToken`
- 不在 Repository 内 catch 异常
