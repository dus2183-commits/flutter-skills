...
description: 维护应用主题配置(颜色/字体/spacing/圆角)。用户说"加一个新颜色"、"改字号"、"新增主题"、"统一 spacing"时触发。改完追加 ADR。
type: skill
stage: 3
model: sonnet
priority: P2
...
- `lib/app/theme/app_spacing.dart`（如果存在）
...
```dart
// 改动: lib/app/theme/app_colors.dart
...
```dart
// 改动: lib/app/theme/app_text_styles.dart
...
```dart
// 改动: lib/app/theme/app_spacing.dart (或统一到 constants 中)
...
> ⚠️ **高频错误警告:**
> - `Color.withOpacity()` 已 deprecated (Flutter 3.27) → 改 `Color.withValues(alpha: 0.15)`
> - 不要硬编码 `Color(0xFF...)`,统一放 AppColors
> - 不要硬编码 `fontSize: 14`,统一放 AppTextStyles
> - 主题路径确认: 检查项目用的是 `lib/app/theme/` 还是 `lib/app/theme/`,以实际为准
